...
namespace yasso {
  ...
  void yasso15::getSpin(const arma::vec5& infall, arma::vec5& result) {
    if(aNeedToBeSet) {
      for(int i=0; i<5; ++i) {
  ...
  void yasso15::getNextTimestep(const arma::vec5& init, const arma::vec5& infall, arma::vec5& result) {
    if(aNeedToBeSet) {
      for(int i=0; i<5; ++i) {
	result[i] = std::numeric_limits<double>::quiet_NaN();
      }
	...
	z1 = Am * init + infall;
	z2 = arma::expmat(Am * timespan) * z1 - infall;
	result = arma::solve(Am,z2);
      }
    }
  }
