#ifndef YASSO_H_EIGEN
#define YASSO_H_EIGEN
...
namespace yasso {
 ...
 //0..Ethanol sol.(waxes), 1..Water sol.(sugar), 2..Acid soluble (Celluloses), 3..Insoluble (lignin), 4..Humus : in init, infall an result
    void getSpin(const Eigen::Matrix<double, 5, 1>& infall, Eigen::Matrix<double, 5, 1>& result);
    void getNextTimestep(const Eigen::Matrix<double, 5, 1>& init, const Eigen::Matrix<double, 5, 1>& infall, Eigen::Matrix<double, 5, 1>& result);
  private:
    double timespan = 1.;
